...
- **The devkit bundles the C++ standard library, runtime and headers.** `libstdc++.so.6` and `libgcc_s.so.1` ship next to the lorelei runtimes (the host `lib/` and the guest `x86_64/lib/`, with their `.so` linker symlinks), and the host libstdc++ headers ship under `lib/cxx/`. So **running** a thunk needs only libc from the host, and **building** one needs only a C compiler for the libc headers, not `g++` or `libstdc++-dev`. A clean container with just a C library and C toolchain drives a devkit plus a thunk pack directly.
