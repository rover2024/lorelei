...
You do not write the per-library glue by hand. The Thunk Library Compiler (TLC), built on Clang LibTooling, reads a library's headers and generates the guest and host thunks, including the awkward cases of callbacks and variadic functions. See [docs/HowToUseTLC.md](docs/HowToUseTLC.md) for how to use it.

[LoreThunk](https://github.com/rover2024/lorelei-thunks) is a companion repository of ready-made thunks (zlib, lzma, ...). It also ships runnable, end-to-end examples that build a thunk and run it under the plugin, so you can see the whole flow working there.
