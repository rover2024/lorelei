...
2. `GuestClient::getThunkInfo(modulePath, false)` 查 forward thunk
...
2. 通过 `HostServer::thunkConfig()` 查 forward 信息
...
1. Thunk config 已从 `HostServer.cpp` 挪到 `HostRT/main.cpp` 加载并注入
