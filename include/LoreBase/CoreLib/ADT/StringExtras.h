...
#include <LoreBase/CoreLib/ADT/ArrayView.h>
...
namespace lore {
    ...
    namespace str {
        ...
        struct conv<std::wstring> {
            ...
            LORECORE_EXPORT static std::wstring from_utf8(const char *s, int size = -1);

            static inline std::wstring from_utf8(const std::string_view &s) {
                return from_utf8(s.data(), int(s.size()));
            }

            LORECORE_EXPORT static std::string to_utf8(const wchar_t *s, int size = -1);
        ...
        struct conv<std::filesystem::path> {
            ...
            LORECORE_EXPORT static std::string normalize_separators(const std::string &utf8_path,
                                                                    bool native);
        };
        ...
        LORECORE_EXPORT std::string join(const ArrayView<std::string> &v,
                                         const std::string_view &delimiter);

        // @overload: join(vector<string_view>, string_view)
        LORECORE_EXPORT std::string join(const ArrayView<std::string_view> &v,
                                         const std::string_view &delimiter);

        LORECORE_EXPORT std::vector<std::string_view> split(const std::string_view &s,
                                                            const std::string_view &delimiter);

        // @overload: split(string &&, string_view)
        LORECORE_EXPORT std::vector<std::string> split(std::string &&s,
                                                       const std::string_view &delimiter);

        LORECORE_EXPORT std::string format(const std::string_view &fmt,
                                           const ArrayView<std::string> &args);
    ...
    namespace str {
        ...
        LORECORE_EXPORT std::string asprintf(const char *fmt, ...);

        LORECORE_EXPORT std::string vasprintf(const char *fmt, va_list args);
