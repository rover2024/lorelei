...
#define FOR_EACH_INLINE_SIZE(body)                                                                 \
    ...
    void checkGrowsAndHolds() {
        ...
        BOOST_VERIFY(v.capacity() == N);
        for (int i = 0; i < 50; ++i)
            v.push_back(i);
        BOOST_VERIFY(v.size() == 50u);
        BOOST_VERIFY(v.capacity() >= 50u);
        for (int i = 0; i < 50; ++i)
            BOOST_VERIFY(v[i] == i);
    }
    ...
    void checkInsertErase() {
        ...
        // {0, 1, 99, 2, 3, 9}
        BOOST_VERIFY(v.size() == 6u);
        BOOST_VERIFY(v[0] == 0 && v[1] == 1 && v[2] == 99 && v[3] == 2 && v[4] == 3 && v[5] == 9);

        v.insert(v.begin() + 1, 3u, 7);  // count form: three 7s after index 0
        // {0, 7, 7, 7, 1, 99, 2, 3, 9}
        BOOST_VERIFY(v.size() == 9u);
        BOOST_VERIFY(v[1] == 7 && v[2] == 7 && v[3] == 7 && v[4] == 1);

        int more[] = {100, 200};
        v.insert(v.begin(), more, more + 2); // range form
        BOOST_VERIFY(v.front() == 100 && v[1] == 200);

        v.erase(v.begin());                       // drop the 100
        BOOST_VERIFY(v.front() == 200);
        v.erase(v.begin(), v.begin() + 1);        // drop the 200
        BOOST_VERIFY(v.front() == 0);

        // Erase the three consecutive 7s.
        v.erase(v.begin() + 1, v.begin() + 4);
        BOOST_VERIFY(v[0] == 0 && v[1] == 1 && v[2] == 99);
    }
    ...
    void checkSwap() {
        ...
        a.swap(b);
        BOOST_VERIFY(a.size() == 5u && a[4] == 50);
        BOOST_VERIFY(b.size() == 3u && b[2] == 3);

        swap(a, b); // free swap()
        BOOST_VERIFY(a.size() == 3u && a[0] == 1);
        BOOST_VERIFY(b.size() == 5u && b[0] == 10);
    }
} // namespace
...
BOOST_AUTO_TEST_CASE(empty_vector) {
    ...
    BOOST_VERIFY(v.size() == 0u);
    BOOST_VERIFY(v.capacity() == 4u);
    BOOST_VERIFY(v.begin() == v.end());
}
...
BOOST_AUTO_TEST_CASE(push_back_and_index) {
    ...
    v.push_back(20);
    v.push_back(30);
    BOOST_VERIFY(v.size() == 3u);
    BOOST_VERIFY(v[0] == 10 && v[1] == 20 && v[2] == 30);
    BOOST_VERIFY(v.front() == 10 && v.back() == 30);
    BOOST_VERIFY(v.data() == &v[0]);
}
...
BOOST_AUTO_TEST_CASE(stays_inline_then_spills_to_heap) {
    ...
    BOOST_VERIFY(v.capacity() == 2u);

    v.push_back(3);
    BOOST_VERIFY(v.size() == 3u);
    BOOST_VERIFY(v.capacity() >= 3u);
    BOOST_VERIFY(v.data() != inlinePtr);
    BOOST_VERIFY(v[0] == 1 && v[1] == 2 && v[2] == 3);
}
...
BOOST_AUTO_TEST_CASE(emplace_back_returns_reference_and_builds_in_place) {
    ...
    BOOST_VERIFY(ref.value == 42);
    BOOST_VERIFY(&ref == &v.back());
    BOOST_VERIFY(Tracked::numConstructed == 1); // no temporary copied/moved
}

BOOST_AUTO_TEST_CASE(resize_grow_shrink_and_value) {
    VarSizeArray<int, 2> v;
    v.resize(5);
    BOOST_VERIFY(v.size() == 5u);
    for (int x : v)
        BOOST_VERIFY(x == 0);
    v.resize(2);
    BOOST_VERIFY(v.size() == 2u);
    v.resize(4, 7);
    BOOST_VERIFY(v.size() == 4u && v[2] == 7 && v[3] == 7);
}
...
BOOST_AUTO_TEST_CASE(clear_keeps_capacity) {
    ...
    v.clear();
    BOOST_VERIFY(v.empty() && v.capacity() == cap);
}

BOOST_AUTO_TEST_CASE(iterator_walk) {
    VarSizeArray<int, 2> v{1, 2, 3, 4};
    BOOST_VERIFY(std::accumulate(v.begin(), v.end(), 0) == 10);
    int expected = 1;
    for (int x : v)
        BOOST_VERIFY(x == expected++);
}
...
BOOST_AUTO_TEST_CASE(insert_initializer_list_and_returned_iterator) {
    ...
    BOOST_VERIFY(v.size() == 4u);
    BOOST_VERIFY(v[0] == 1 && v[1] == 2 && v[2] == 3 && v[3] == 4);
    BOOST_VERIFY(it == v.begin() + 1 && *it == 2);

    auto e = v.erase(v.begin() + 1);
    BOOST_VERIFY(*e == 3 && v.size() == 3u);
}
...
BOOST_AUTO_TEST_CASE(swap_heap_with_heap_trades_buffers) {
    ...
    const int *pb = b.data();
    a.swap(b);
    BOOST_VERIFY(a.data() == pb && b.data() == pa); // pointers traded, no element moves
    BOOST_VERIFY(a.size() == 4u && b.size() == 3u);
}
...
BOOST_AUTO_TEST_CASE(swap_inline_with_heap) {
    ...
    inl.swap(hp);
    BOOST_VERIFY(inl.size() == 5u && inl[4] == 5);
    BOOST_VERIFY(hp.size() == 2u && hp[0] == 1 && hp[1] == 2);
}
...
BOOST_AUTO_TEST_CASE(swap_balances_lifetimes) {
    Tracked::reset();
    {
        ...
        a.swap(b);
        BOOST_VERIFY(a.size() == 1u && a[0].value == 100);
        BOOST_VERIFY(b.size() == 3u && b[2].value == 2);
    }
    BOOST_VERIFY(Tracked::alive() == 0);
}
...
BOOST_AUTO_TEST_CASE(copy_is_deep) {
    ...
    b[0] = 99;
    BOOST_VERIFY(a[0] == 1 && b[0] == 99 && b.size() == 3u);
    VarSizeArray<int, 2> c;
    c = a;
    BOOST_VERIFY(c.size() == 3u && c[2] == 3);
}
...
BOOST_AUTO_TEST_CASE(move_steals_heap_buffer) {
    ...
    BOOST_VERIFY(b.size() == 4u && b.data() == heap);
    BOOST_VERIFY(a.empty());
    BOOST_VERIFY(b[3] == 4);
}
...
BOOST_AUTO_TEST_CASE(move_from_inline_transfers_elements) {
    ...
    BOOST_VERIFY(b.size() == 2u && b[0] == 1 && b[1] == 2);
    BOOST_VERIFY(a.empty());
}
...
BOOST_AUTO_TEST_CASE(base_reference_is_inline_size_agnostic) {
    ...
    BOOST_VERIFY(sum(small) == 6);
    BOOST_VERIFY(sum(big) == 30);
}
...
BOOST_AUTO_TEST_CASE(cross_inline_size_assignment_and_swap) {
    ...
    BOOST_VERIFY(dst.size() == 3u && dst[0] == 7 && dst[2] == 9);

    VarSizeArray<int, 2> moved = std::move(src);
    BOOST_VERIFY(moved.size() == 3u && moved[1] == 8);
    ...
    BOOST_VERIFY(x.size() == 3u && x[2] == 5);
    BOOST_VERIFY(y.size() == 2u && y[1] == 2);
}
...
BOOST_AUTO_TEST_CASE(element_lifetimes_are_balanced) {
    Tracked::reset();
    {
        ...
        for (int i = 0; i < 10; ++i)
            a.emplace_back(i);
        BOOST_VERIFY(a.size() == 10u && a[9].value == 9 && Tracked::alive() == 10);

        VarSizeArray<Tracked, 2> b(std::move(a));
        VarSizeArray<Tracked, 4> c = b;
        c.insert(c.begin() + 1, Tracked(77));
        c.erase(c.begin());
        BOOST_VERIFY(Tracked::alive() == 20 + 1 - 1);
    }
    BOOST_VERIFY(Tracked::alive() == 0); // no leaks or double frees
}
...
BOOST_AUTO_TEST_CASE(self_assignment_is_a_noop) {
    ...
    ref = ref;
    BOOST_VERIFY(v.size() == 3u && v[0] == 1 && v[2] == 3);
    ref = std::move(ref);
    BOOST_VERIFY(v.size() == 3u && v[2] == 3);
}

BOOST_AUTO_TEST_CASE(zero_inline_is_pure_heap) {
    VarSizeArray<int, 0> v;
    BOOST_VERIFY(v.capacity() == 0u && v.empty());
    v.push_back(5);
    BOOST_VERIFY(v.size() == 1u && v[0] == 5 && v.capacity() >= 1u);
}
