...
The plan: package **ThunkExample** as a devkit thunk under `lorelei-thunks/examples/` (alongside `hello`) — a `ThunkExample.toml` plus its host implementation — so that on any host arch `MakeThunk.py --devkit-prefix ... --config ThunkExample.toml` builds the HTL native and the GTL for x86_64, and the same `qemu -L <devkit>/x86_64/sysroot -plugin libdlcall.so` run drives the program. See [`docs/claude/emulation.md`](../../../../docs/claude/emulation.md) for the full devkit flow. To be done once the `MakeThunk` flow is fully exercised.
